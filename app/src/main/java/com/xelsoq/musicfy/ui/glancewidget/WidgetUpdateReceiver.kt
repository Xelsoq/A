...
class WidgetUpdateReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        if (intent.action != "com.xelsoq.musicfy.ACTION_WIDGET_UPDATE_PLAYBACK_STATE") return
        ...
        scope.launch {
            try {
                ...
                val glanceIds = glanceAppWidgetManager.getGlanceIds(MusicfyGlanceWidget::class.java)
                glanceIds.forEach { glanceId ->
                    MusicfyGlanceWidget().update(context, glanceId)
                }
