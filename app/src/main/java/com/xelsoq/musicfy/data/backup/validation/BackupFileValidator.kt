package com.xelsoq.musicfy.data.backup.validation
...
import com.xelsoq.musicfy.data.backup.model.Severity
...
class BackupFileValidator @Inject constructor(
    ...
    fun validate(uri: Uri): BackupValidationResult {
        ...
        if (format == BackupFormatDetector.Format.UNKNOWN) {
            errors.add(ValidationError("FORMAT_UNKNOWN", "File is not a recognized Musicfy backup format."))
            return BackupValidationResult.Invalid(errors)
        }
