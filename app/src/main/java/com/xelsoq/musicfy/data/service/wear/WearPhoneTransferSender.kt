...
import com.xelsoq.musicfy.shared.WearCapabilities
...
class WearPhoneTransferSender @Inject constructor(
    ...
    suspend fun isMusicfyWatchAvailable(): Boolean {
        return runCatching {
            val capability = capabilityClient.getCapability(
                WearCapabilities.MUSICFY_WEAR_APP,
            ...
            Timber.tag(TAG).w(error, "Failed checking Musicfy Wear availability")
            false
        }
    }
    ...
    suspend fun requestSongTransfer(songId: String, songTitle: String = ""): Result<Int> {
        ...
        return runCatching {
            ...
            if (nodes.isEmpty()) {
                error("No reachable watch with Musicfy")
            }
