...
class ImageCacheManager @Inject constructor(
    ...
    fun invalidateCoverArtCaches(vararg uriStrings: String?) {
        ...
        uriStrings.mapNotNull { it?.takeIf(String::isNotBlank) }.forEach { baseUri ->
            if (com.xelsoq.musicfy.utils.LocalArtworkUri.isLocalArtworkUri(baseUri)) {
                com.xelsoq.musicfy.utils.LocalArtworkUri.parseSongId(baseUri)?.let { songId ->
                    com.xelsoq.musicfy.utils.AlbumArtUtils.clearCacheForSong(context, songId)
                }
            }
