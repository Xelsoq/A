...
fun SongItem.toNativeSong(): Song {
    ...
    val artistNames = com.xelsoq.musicfy.data.stream.CloudMusicUtils.parseArtistNames(rawArtistName)
...
fun com.xelsoq.musicfy.data.model.youtube.Song.toNativeSong(): Song {
    val artistNames = com.xelsoq.musicfy.data.stream.CloudMusicUtils.parseArtistNames(artist)
    val artistRefs = artistNames.mapIndexed { index, name ->
        val artistId = -(17_000_000_000_000L + kotlin.math.abs(name.lowercase().hashCode().toLong()))
        ArtistRef(
            id = artistId,
