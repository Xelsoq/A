package com.xelsoq.musicfy.presentation.components.scoped
...
internal fun rememberSheetVisualState(
    ...
    val baseBottomPadding = remember(containerHeight, sheetCollapsedTargetY, density) {
        ...
        (containerHeight - com.xelsoq.musicfy.presentation.components.MiniPlayerHeight - targetYDp)
            .coerceAtLeast(0.dp)
    }

    val currentBottomPadding = 0.dp

    val miniHeightPx = remember(density) { with(density) { com.xelsoq.musicfy.presentation.components.MiniPlayerHeight.toPx() } }
