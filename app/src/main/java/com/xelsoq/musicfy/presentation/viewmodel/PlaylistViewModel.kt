...
import com.xelsoq.musicfy.data.DailyMixManager
import com.xelsoq.musicfy.data.model.Playlist
import com.xelsoq.musicfy.data.model.SmartPlaylistRule
import com.xelsoq.musicfy.data.model.Song
import com.xelsoq.musicfy.data.model.SortOption
import com.xelsoq.musicfy.data.playlist.M3uManager
...
import com.xelsoq.musicfy.data.preferences.TelegramTopicDisplayMode
import com.xelsoq.musicfy.data.ai.AiPlaylistGenerator
...
class PlaylistViewModel @Inject constructor(
                    ...
                    currentPlaylist.coverImageUri?.let { oldPath ->
                        if (oldPath.contains("playlist_cover_")) {
                            try { File(oldPath).delete() } catch (e: Exception) {
                                Log.w("PlaylistViewModel", "Failed to delete old cover file: $oldPath", e)
                            }
                        }
                    }
    ...
    private fun findFolder(
        targetPath: String,
        folders: List<com.xelsoq.musicfy.data.model.MusicFolder>
    ): com.xelsoq.musicfy.data.model.MusicFolder? {
        val queue: ArrayDeque<com.xelsoq.musicfy.data.model.MusicFolder> = ArrayDeque(folders)
    ...
    private fun com.xelsoq.musicfy.data.model.MusicFolder.collectAllSongs(): List<Song> {
    ...
    private fun sortPlaylistsList(
        playlists: List<com.xelsoq.musicfy.data.model.Playlist>,
        sortOption: SortOption
    ): List<com.xelsoq.musicfy.data.model.Playlist> {
        return when (sortOption) {
            SortOption.PlaylistNameAZ -> playlists.sortedWith(
                compareBy<com.xelsoq.musicfy.data.model.Playlist> { it.name.lowercase() }
                    .thenByDescending { it.lastModified }
                    .thenBy { it.id }
            )
            SortOption.PlaylistNameZA -> playlists.sortedWith(
                compareByDescending<com.xelsoq.musicfy.data.model.Playlist> { it.name.lowercase() }
                    .thenByDescending { it.lastModified }
                    .thenBy { it.id }
            )
            SortOption.PlaylistDateCreated -> playlists.sortedWith(
                compareByDescending<com.xelsoq.musicfy.data.model.Playlist> { it.lastModified }
                    .thenBy { it.name.lowercase() }
                    .thenBy { it.id }
            )
            SortOption.PlaylistDateCreatedAsc -> playlists.sortedWith(
                compareBy<com.xelsoq.musicfy.data.model.Playlist> { it.lastModified }
                    .thenBy { it.name.lowercase() }
                    .thenBy { it.id }
            )
    ...
    fun exportPlaylistsAsM3u(playlistIds: List<String>) {
        ...
        viewModelScope.launch {
            try {
                ...
                val exportDir = File(musicDir, "Musicfy Exports")
                if (!exportDir.exists()) {
                    exportDir.mkdirs()
                }
