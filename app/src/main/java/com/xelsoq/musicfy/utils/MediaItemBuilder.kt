...
object MediaItemBuilder {
    ...
    private const val EXTERNAL_EXTRA_PREFIX = "com.xelsoq.musicfy.external."
    ...
    fun buildForExternalController(context: Context, song: Song): MediaItem {
        ...
        return com.xelsoq.musicfy.data.diagnostics.PerformanceMetrics.time(
            com.xelsoq.musicfy.data.diagnostics.PerformanceMetrics.Timings.MEDIASESSION_ITEM_BUILD
        ) {
            MediaItem.Builder()
                .setMediaId(song.id)
                .setUri(playbackUri(song))
